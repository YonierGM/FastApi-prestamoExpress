...
@prestamoRoutes.get("/prestamos/{id}", tags=["prestamos"], response_model=Prestamo,
...
@prestamoRoutes.get("/prestamoscliente/{id}", tags=["prestamos"], response_model=List[Prestamo],
                    description="Se buscara los prestamos del cliente")
def get_prestamocliente(id: int):
    existing_prestamo = conn.execute(
        prestamos.select().where(prestamos.c.clienteid == id).order_by(prestamos.c.estadoid)).fetchall()
...
def create_prestamo(prestamo: Prestamo):
    try:
        new_prestamo = {"fechaprestamo": prestamo.fechaprestamo, "fechaestimadapago": prestamo.fechaestimadapago,
                        ...
                        "clienteid": prestamo.clienteid, "estadoid": prestamo.estadoid,
